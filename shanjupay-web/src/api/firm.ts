...
//商户个人认证审核
export const OneApprove = (auditStatus:any,id:string) =>
  request({
    url: `/operation/m/merchants/audit/${auditStatus}?merchantId=${id}`,
    method: 'post'
  })
...
request({
  url: `/operation/m/staffs/page?pageNo=${pageNo}&pageSize=${pageSize}`,
  method: 'get'
})
...
request({
  url: `/operation/m/merchants?merchantId=${UserModule.tenantId}`,
  method: 'DELETE'
})
