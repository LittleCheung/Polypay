...
//生成商户应用门店二维码
export const getStoresDataCode =  (appId:string,storeId:string) => 
    request ({
...
//生成c扫b二维码
export const getCsaoBCode =  (appId:string,storeId:string,body:string,totalAmount:string) => 
    request ({
        url:`/merchant/my/apps/${appId}/stores/${storeId}/order-qrcode?tenantId=${UserModule.tenantId}&body=${body}&totalAmount=${totalAmount}`,
        method:'get'
    })
//应用财务对账
export const useMoney =  () => 
    request ({
        url:`/merchant/my/merchants/reconciliations?tenantId=${UserModule.tenantId}`,
        method:'post'
    })
...
request ({
    url:`/merchant/my/merchants/transactiondetails?tenantId=${UserModule.tenantId}&transactionTime=${transactionTime}`,
    method:'post'
})