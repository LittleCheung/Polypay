...
class User extends VuexModule implements IUserState {
  ...
  @Mutation
  private SET_CODES(codes: string[]) {
    this.codes = codes
  }
  ...
  public async Login(userInfo: { usernames: string; password: string }) {
    ...
    setTenantId(arr[0])
    // let a =
    // let a = responses.payload[arr[0]]

    // let as = a.user_authorities.r_001

    // console.log(responses.payload[a].user_authorities.r_001)
  ...
  @Action
  public async operationLogin(userInfo: {
    usernames: string
    password: string
  }) {
    let { usernames, password } = userInfo
  ...
  public async GetUserInfo() {
    ...
    var cookie = Cookies.get('vue_typescript_admin_access_token')
    const param = new URLSearchParams()
    param.append('token', cookie!)
    let responses = await checkToken(param)
    let arr = []
    for (var item in responses.payload) {
      arr.push(item)
    }
    ...
    let as = []
    if (responses.client_id === 'merchant-platform') {
      let a = responses.payload[arr[0]]
      as = a.user_authorities.r_001
    } else {
      let a = responses.payload[arr[0]]
      as = a.user_authorities.r_sj_operation_admin
    }

    // let s = localStorage.getItem('codes');
    //       let codess = JSON.parse(s)
    // console.log(codess)
    this.SET_MENUS(as)
  ...
  public async LogOut() {
    ...
    Cookies.remove('vue_typescript_admin_access_token')
